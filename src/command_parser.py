...
class CommandParser:
    ...
    def execute_command(self, command: Sequence[str],fix,con,playlists):
        ...
        if command[0].upper() == "NUMBER_OF_VIDEOS":
            ...
            fix=self._player.play_video(command[1],fix)

        elif command[0].upper() == "PLAY_RANDOM":
            fix=self._player.play_random_video(fix)

        elif command[0].upper() == "STOP":
            fix=self._player.stop_video(fix)

        elif command[0].upper() == "PAUSE":
            con=self._player.pause_video(fix,con)

        elif command[0].upper() == "CONTINUE":
            con=self._player.continue_video(fix,con)

        elif command[0].upper() == "SHOW_PLAYING":
            self._player.show_playing(fix)
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "playlist name.")
            playlists=self._player.create_playlist(command[1],playlists)
            ...
            playlists=self._player.add_to_playlist(command[1], command[2],playlists)
            ...
            playlists=self._player.remove_from_playlist(command[1], command[2,playlists])
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "playlist name.")
            playlists=self._player.clear_playlist(command[1],playlists)
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "playlist name.")
            playlists=self._player.delete_playlist(command[1],playlists)
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "playlist name.")
            self._player.show_playlist(command[1],playlists)

        elif command[0].upper() == "SHOW_ALL_PLAYLISTS":
            self._player.show_all_playlists(playlists)
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "search term.")
            fix=self._player.search_videos(command[1],fix)
